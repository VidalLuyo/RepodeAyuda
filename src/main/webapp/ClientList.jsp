<%@ page import="pe.edu.vallegrande.SisVentas.dto.Clientes" %>
<%@ page import="pe.edu.vallegrande.SisVentas.Controller.ClienteController" %>
    ...
    <div id="layoutSidenav_content">
        <main role="main" class="container-fluid">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form id="searchForm" class="form-inline" action="listadoClientes.jsp" method="GET">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <%
                                ...
                                List<Clientes> clientes = clienteDAO.listarActivos();

                                if (clientes.isEmpty()) {
                            %>
                            <tr>
                                <td colspan="12" class="text-center">No hay clientes disponibles por el momento.</td>
                            </tr>
                            <%
                            } else {
                            ...
                            %>
                            <tr>
                                <td class="text-center"><%= cliente.getClientCode() %></td>
                                <td class="text-center"><%= cliente.getDocumentType() %></td>
                                <td class="text-center"><%= cliente.getDocumentNumber() %></td>
                                <td class="text-center"><%= cliente.getFirstName() %></td>
                                <td class="text-center"><%= cliente.getLastName() %></td>
                                <td class="text-center"><%= cliente.getBirthDate() %></td>
                                <td class="text-center"><%= cliente.getEmail() %></td>
                                <td class="text-center"><%= cliente.getPhone() %></td>
                                <td class="text-center"><%= cliente.getPhone2() %></td>
                                <td class="text-center"><%= cliente.getAddress() %></td>
                                <td class="text-center">
                                    <!-- Botón Editar -->
                                    <a href="editarCliente.jsp?id=<%= cliente.getClientId() %>" class="btn btn-primary btn-sm">Editar</a>
                                    <!-- Botón Eliminar -->
                                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmDelete(<%= cliente.getClientId() %>)">Eliminar</button>
                                </td>
                            </tr>
                            <%
                                    }
